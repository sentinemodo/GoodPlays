# Restart local dev, redeploy GitHub Pages from main, and require the public laptop API to be Healthy.
...
Write-Host "Checking public API health at $($script:PublicApiHealthUrl)"
$publicOk = Wait-Until -TimeoutSec 120 -IntervalSec 5 -Condition { Test-ServiceHealthy $script:PublicApiHealthUrl }
if (-not $publicOk) {
    Write-Host "PUBLIC_API_UNHEALTHY $($script:PublicApiHealthUrl)"
    Write-Host 'RESULT: public-api-unhealthy'
    exit 2
}

Write-Host "PUBLIC_API: $($script:PublicApiHealthUrl) Healthy"
